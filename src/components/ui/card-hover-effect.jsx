...
export const BentoCard = ({ item }) => {
  return (
    <a href={item?.link} className="block group relative z-20">
      <div className="relative bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm h-96 transition-all duration-700 ease-out group-hover:shadow-2xl group-hover:shadow-blue-500/15 group-hover:-translate-y-2 group-hover:border-gray-200">
        ...
        {/* Glow + Shimmer */}
        <div className="absolute inset-0 bg-gradient-to-br from-blue-500/8 via-purple-500/5 to-pink-500/8 opacity-0 group-hover:opacity-100 transition-opacity duration-700 rounded-3xl z-10"></div>
        <div className="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 bg-gradient-to-r from-transparent via-white/20 to-transparent skew-x-12 opacity-0 group-hover:opacity-100 z-10"></div>
      </div>
    </a>
  );
};
